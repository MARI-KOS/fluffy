...
export function initStableFluids(container: HTMLElement): CleanupFn {
  if (!container) {
...
`;
  ...
  function updateResolution() {
    const pixelRatio = renderer.getPixelRatio();
    resolution.set(Math.floor(window.innerWidth * pixelRatio), Math.floor(window.innerHeight * pixelRatio));
  }
  updateResolution();
  ...
  function updateMouse(e: { clientX: number; clientY: number }) {
  ...
  function makeQuad(fragmentShader: string, uniforms: Record<string, { value: unknown }> = {}) {
